...
namespace LibCSG{
    ...
    **/
    public class CSGBrush
    {
        ...
        public Mesh getMeshNormal(Mesh m = null)
        {
            if (m == null)
            {
                m = new Mesh();
            ...
            Vector2[] uv = new Vector2[faces.Length * 3];
            int[] triangles = new int[faces.Length * 3];
            float minx = Mathf.Infinity, maxx = -Mathf.Infinity, miny = Mathf.Infinity, maxy = -Mathf.Infinity, minz = Mathf.Infinity, maxz = -Mathf.Infinity;
            for (int i = 0; i < faces.Length; i++)
            {
                for (int l = 0; l < 3; l++)
                {
                    minx = Mathf.Min(minx, faces[i].vertices[l].x);
                    maxx = Mathf.Max(maxx, faces[i].vertices[l].x);
                    miny = Mathf.Min(miny, faces[i].vertices[l].y);
                    maxy = Mathf.Max(maxy, faces[i].vertices[l].y);
                    minz = Mathf.Min(minz, faces[i].vertices[l].z);
                    maxz = Mathf.Max(maxz, faces[i].vertices[l].z);
                }
            }
            Vector3 v = new Vector3((maxx + minx) / 2f, (maxy + miny) / 2f, (maxz + minz) / 2f);

            for (int i = 0; i < faces.Length; i++)
            {
                vert[3 * i + 2] = faces[i].vertices[0] - v;
                vert[3 * i + 1] = faces[i].vertices[1] - v;
                vert[3 * i] = faces[i].vertices[2] - v;
                triangles[3 * i] = 3 * i;
                triangles[3 * i + 1] = 3 * i + 1;
                triangles[3 * i + 2] = 3 * i + 2;
                uv[3 * i + 2] = faces[i].uvs[0];
